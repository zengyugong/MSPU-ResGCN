...
from tf_lib.gcn_lib.vertex import nodeshuffle, multi_cnn, duplicate, mlpshuffle, inception_densegcn, densegcn, gcn, densemsgcn,inception_densemsgcn
...
# Feature Extraction Block used in point cloud upsanmpling network
# ------------------------------------------------------------
def feature_block_updated(x,
                  block='inception_ms',
                  k=16, d=2, growth_rate=12, n_dense=3,num_scales=3,
                  kernel_sizes=[1, 3, 5],
                  use_global_pooling=True,
                  scope='FEB',
                  **kwargs):
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        # this is the default version of Inception DenseGCN in our paper PU-GCN
        if block == 'inception_ms':
            y = inception_densemsgcn(x, growth_rate=growth_rate, k=k, d=d, n_dense=n_dense,num_scales=num_scales,
                                   kernel_sizes=kernel_sizes,
                                   use_global_pooling=use_global_pooling,
                                   scope='inception_ms', **kwargs)
...
def feature_extractor(inputs,
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        l0_features = tf.expand_dims(inputs, axis=2)
        hidden_features, idx0 = densemsgcn(l0_features, growth_rate=growth_rate,
                                         n_layers=1, k=k, d=1,
...
# Feature Extractor used in point cloud upsampling network
# ----------------------------------------------- -------------
def feature_extractor_updated(inputs,
                      block='inception_ms', n_blocks=3,
                      growth_rate=24, k=16, d=2,
                      n_dense=3,
                      num_scales=3,
                      kernel_sizes=[1, 3, 5],
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        feat_fuse = 0.
        for i in range(0, n_blocks):
            hidden_features = feature_block_updated(hidden_features,
                                            block=block,
                                            k=k, d=d, growth_rate=growth_rate, n_dense=n_dense,num_scales=num_scales,
                                            kernel_sizes=kernel_sizes,
                                            use_global_pooling=use_global_pooling,
                                            scope='FEB_' + str(i),
                                            is_training=is_training, bn_decay=bn_decay, use_bn=use_bn, use_ibn=use_ibn
...
def pairwise_distance(point_cloud):
    """Compute pairwise distance for a batch of point clouds."""
    point_cloud_transpose = tf.transpose(point_cloud, perm=[0, 2, 1])  # Shape: [B, 3, N]
    point_cloud_inner = tf.matmul(point_cloud, point_cloud_transpose)  # [B, N, N]

    point_cloud_sq = tf.reduce_sum(tf.square(point_cloud), axis=-1, keepdims=True)
    point_cloud_sq_transpose = tf.transpose(point_cloud_sq, perm=[0, 2, 1])  # [B, N, 1] -> [B, 1, N]

    pairwise_dist = point_cloud_sq - 2 * point_cloud_inner + point_cloud_sq_transpose
    pairwise_dist = tf.maximum(pairwise_dist, 0.0)  # Ensure non-negative values for stability

    return pairwise_dist



def knn(pairwise_dist, k):
    """Find the k-nearest neighbors based on pairwise distances."""

    # Get the top k smallest distances (nearest neighbors).
    _, idx = tf.nn.top_k(-pairwise_dist, k=k)  # Negative distances to get smallest
